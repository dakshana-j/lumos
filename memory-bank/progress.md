...
### Phase 11 -- PR Review Workflow (Complete, branch: feat/review-pr-mode)

Adds a third orchestrator entry point (`reviewPr()`) for structured PR quality
checks, decoupled from both failure analysis and test generation.

1. **Dedicated PR review prompt module** (`src/prompts/pr-review-prompt.ts`):
   - `buildPrReviewSystemPrompt()` — repo-agnostic role, 6-step workflow,
     10 review checks with PASS/FAIL/SKIP semantics, rigid comment template
   - `buildPrReviewUserMessage()` — compact: workspace, repository, PR ID,
     optional trigger source

2. **Ten structured review checks** (all hard-fail except 5–6 which are soft):
   - **1** — PR Description Completeness (Problem / Root Cause / Solution /
     How to Test, >100 chars, no placeholder text)
   - **2** — PR Title Convention (ticket ref + valid type, repo-agnostic)
   - **3** — Build/CI Status (PASS/FAIL/SKIP based on MCP data)
   - **4** — Test Coverage (testable files changed → test files touched)
   - **5** — Playwright Convention Compliance (spec location, isMockingEnabled
     placement, CI-only skips) — soft-fail
   - **6** — Automation Coverage (both mock and non-mock paths present) — soft-fail
   - **7** — How to Test (Dev-Authored): section must NOT be auto-generated by
     AI tools (Yama, Copilot, etc.). Detection signals: generic boilerplate,
     no feature-specific detail, LLM-style phrasing, missing concrete values.
   - **8** — Video Proof — Mocking Mode: screen recording/GIF/link showing
     feature under mocked backend. SKIP for non-UI changes only.
   - **9** — Video Proof — Non-Mocking Mode: separate recording against real
     backend/staging. Both 8 and 9 must be present. SKIP for non-UI only.
   - **10** — Dev Code-Change Proof: screenshots, before/after, logs, or
     recording showing developer manually tested the change. SKIP for pure
     refactors/docs/type-only changes with justification.
...
4. **New orchestrator flow** (`src/orchestrator.ts`):
   ...
   `this.neurolink.generate()` call, detects `add_comment` via `toolsUsed`,
   calculates token usage/cost, and returns a `ReviewPrResult`.

5. **Public API widened** (`src/index.ts`):
   `createLumos()` returns `{ analyze, generateTests, reviewPr }`.
   Re-exports `ReviewPrOptions`, `ReviewPrResult`, prompt builders.

6. **Type surface expanded** (`src/parsers/types.ts`):
   `ReviewPrOptions`, `ReviewPrResult` added.

7. **Local smoke script** (`scripts/review-local.ts`):
   Env-driven manual runner for `reviewPr()`.

8. **Full test suite** (`test/pr-review.test.ts`, 556 → ~800 lines):
   - `checksRun` assertion updated: 6 → 10
   - Comment fixtures updated to 10-row tables
   - New `describe` block "reviewPr — new checks (checks 7–10)" with 5 tests:
     auto-generated How to Test, missing mocking video, missing non-mocking
     video, missing dev proof, non-UI SKIP path.
...
- Dedicated `reviewPr()` unit/integration coverage
- Real PR validation run for the local review smoke script
...
| find-by-branch verification fix               | lumos      | Done (1.1.2) | --                  |
| Orchestrator cleanup + signal fix             | lumos      | Done (1.1.3) | --                  |
| PR review workflow (10 checks, repo-agnostic) | lumos      | Done         | Pending publish     |
| `reviewPr()` automated tests                  | lumos      | Done         | --                  |
